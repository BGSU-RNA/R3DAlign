...
function [] = rSuperimposeNucleotides(File1,NTList1,File2,NTList2,ViewParam,L,Filename)
...
if VP.Write > 0,
  ...
  if isequal(class(NTList1),'double')
      L1=[num2str(Indices1(1)) '-' num2str(Indices1(end))];
      L2=[num2str(Indices2(1)) '-' num2str(Indices2(end))];
  else
      L1 = strrep(NTList1{1},':','-');
      L2 = strrep(NTList2{1},':','-');
  end
...
fprintf(fid,'MODEL        1\n'); % check spaces
  for k=1:L,                            % Loop through all nucleotides
    a = zWriteNucleotidePDB(fid,File1.NT(Indices1(k)),a,0,R,CC1);
...
fprintf(fid,'MODEL        3\n'); % check spaces
  for k=1:L,                            % Loop through all nucleotides
    a = zWriteNucleotidePDB(fid,File2.NT(Indices2(k)),a,0,SuperR*R,CC2);
