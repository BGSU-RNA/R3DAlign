...
function [A,B] = rBarDiagram(File1,NTList1,File2,NTList2,AlignedNTList1,AlignedNTList2,filename,T,SaveLocation)
...
if nargin == 2 %two arguments --> final alignment .mat file and location to save
   ...
   D=regexp(File1,'--');
   if isempty(M)
      filename = File1(L(1)-4:end);
   else
      filename = File1(L(1)-4:M(1)-1);
   end
...
if length(filename) > 0,
  ...
  if exist(SaveLocation) ~= 0 %#ok<EXIST>
      BarName = fullfile(SaveLocation,BarName);
  end
...
%     saveas(gcf,BarName,'pdf');
  ...
  saveas(gcf,[BarName(1:end-4) '_int'],'pdf')
%   saveas(gcf,[BarName(1:end-4) '_int'],'png')
end
